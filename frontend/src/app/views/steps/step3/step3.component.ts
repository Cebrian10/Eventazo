...
export class Step3Component implements AfterViewInit {
  ...
  @Input() eventoId: string | null = null; // Recibe el evento ID como Input
  @Input() boletos: any[] = []; // Recibe los boletos seleccionados como Input
...
private renderPayPalButton() {
  ...
  private registerTransaction(paypalTransactionId: string, payerName: string, payerEmail: string) {
    ...
    const transactionData = {
        ...
        currency: 'USD',
        status: 'COMPLETED',
        eventoId: Number(this.eventoId), // Convertir a número
        usuarioId: this.userId, // Asegurarse de que es un número
    ...
    console.log('Sending transaction data:', transactionData); // Añadido para depuración

    this.apiService.registrarTransaccion(transactionData).subscribe(
        response => {
            console.log('Transaction registered:', response);
            // Aquí podrías agregar lógica adicional para manejar la respuesta
        },
        error => {
